...
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test1a.fasta" -r "${output_dir}/test1a.tsv" -v -m 2> "${output_dir}/test1a.log"; echo $? > "${output_dir}/test1a.exit"
# Test 1b: midpoint rotate, deleting sequence comment lines (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test1b.fasta" -r "${output_dir}/test1b.tsv" -v -m -s 2> "${output_dir}/test1b.log"; echo $? > "${output_dir}/test1b.exit"
# Test 1c: target specific sequences (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test1c.fasta" -r "${output_dir}/test1c.tsv" -v -m -n "sequence2,sequence3" 2> "${output_dir}/test1c.log"; echo $? > "${output_dir}/test1c.exit"
...
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2a.fasta" -r "${output_dir}/test2a.tsv" -v -p 1 2> "${output_dir}/test2a.log"; echo $? > "${output_dir}/test2a.exit"
# Test 2b: rotate specific sequences (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2b.fasta" -r "${output_dir}/test2b.tsv" -v -p 1 -n "sequence2,sequence3" 2> "${output_dir}/test2b.log"; echo $? > "${output_dir}/test2b.exit"
# Test 2c: rotate to position 0 (should not rotate) (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2c.fasta" -r "${output_dir}/test2c.tsv" -v -p 0 2> "${output_dir}/test2c.log"; echo $? > "${output_dir}/test2c.exit"
# Test 2d: rotate to negative position (should fail)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2d.fasta" -r "${output_dir}/test2d.tsv" -v -p -1 2> "${output_dir}/test2d.log"; echo $? > "${output_dir}/test2d.exit"
# Test 2e: rotate to position larger than all sequences (should fail)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2e.fasta" -r "${output_dir}/test2e.tsv" -v -p 100 2> "${output_dir}/test2e.log"; echo $? > "${output_dir}/test2e.exit"
# Test 2f: rotate to position larger than some sequences (should fail)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2f.fasta" -r "${output_dir}/test2f.tsv" -v -p 10 2> "${output_dir}/test2f.log"; echo $? > "${output_dir}/test2f.exit"
# Test 2g: rotate to the same length as the shortest contig (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test2g.fasta" -r "${output_dir}/test2g.tsv" -v -p 8 2> "${output_dir}/test2g.log"; echo $? > "${output_dir}/test2g.exit"

# Test series 3: rotate by common fraction
# Test 3a: normal rotation by fraction (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test3a.fasta" -r "${output_dir}/test3a.tsv" -v -P 0.3 2> "${output_dir}/test3a.log"; echo $? > "${output_dir}/test3a.exit"
...
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test4a.fasta" -r "${output_dir}/test4a.tsv" -v -t "${guide_positions}" 2> "${output_dir}/test4a.log"; echo $? > "${output_dir}/test4a.exit"
...
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test4b.fasta" -r "${output_dir}/test4b.tsv" -v -t "${guide_positions}" -n "sequence2,sequence3" 2> "${output_dir}/test4b.log"; echo $? > "${output_dir}/test4b.exit"
# Test 4c: rotate by fraction via a guide file (should finish)
spokewrench rotate -i "${input_fasta}" -o "${output_dir}/test4c.fasta" -r "${output_dir}/test4c.tsv" -v -T "${guide_fractions}" 2> "${output_dir}/test4c.log"; echo $? > "${output_dir}/test4c.exit"

set -e

# See if all expected output files were generated
find "${output_dir}" -name "*.fasta" -or -name "*.tsv" -or -name "*.exit" | grep -v "copied" | sed -e "s|^${output_dir}/||g" | sort -h > "${output_dir}/outputs.list"
find "${expected_dir}" -name "*.fasta" -or -name "*.tsv" -or -name "*.exit" | grep -v "copied" | sed -e "s|^${expected_dir}/||g" | sort -h > "${output_dir}/expected.list"
set +e
cmp_status=$(cmp "${output_dir}/outputs.list" "${output_dir}/expected.list" >/dev/null 2>&1 && echo $?)
set -e
if [[ "${cmp_status}" != 0 ]]; then
  # TODO - if any single run fails, some files like the output .fasta will not be generated, causing an error. Need to make the testing more specific.
